...
namespace sdds
{
	...
	int ProcessData::operator()(const std::string& target_file, double& avg, double& var) {
		...
		for (int i = 0; i < num_threads; ++i) {
			...
			int size = end - start;
			avgThreads.emplace_back(std::bind(&computeAvgFactor, data + start, size, total_items, std::ref(averages[i])));
		}

		for (auto& t : avgThreads) {
			t.join();
		}
		...
		for (int i = 0; i < num_threads; ++i) {
			...
			int end = p_indices[i + 1];
			int size = end - start;
			varThreads.emplace_back(std::bind(&computeVarFactor, data + start, size, total_items, avg, std::ref(variances[i])));
		}

		for (auto& t : varThreads) {
			t.join();
		}
		...
		std::ofstream writer(target_file, std::ios::binary);
		if (writer) {
			writer.write(reinterpret_cast<const char*>(&total_items), sizeof(total_items));
			writer.write(reinterpret_cast<const char*>(data), sizeof(int) * total_items);
		}
		else {
			throw std::exception();
		}
